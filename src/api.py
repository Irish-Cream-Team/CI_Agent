...
class API:
    ...
    def __get_project_pipelines(self) -> List[Dict[str, str]]:
        ...
        return requests.request("GET", url, headers=headers, data={}, verify = false).json().get('value')
    ...
    def _send_pipeline_run(self, pipeline_id: str) -> requests.Response:
        ...
        return requests.request("POST", url, headers=headers, data=payload,verify = false).json()
